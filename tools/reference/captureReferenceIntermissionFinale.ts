...
export type ReferenceIntermissionFinaleTerminationCause = 'natural-exit' | 'sandbox-killed';

export type AttractLoopPhase = 'demo-one' | 'demo-three' | 'demo-two' | 'post-demo-one-interstitial' | 'post-demo-three-interstitial' | 'post-demo-two-interstitial' | 'title';

export interface AttractLoopCheckpointContract {
  readonly description: string;
  readonly phase: AttractLoopPhase;
  readonly scheduledAtElapsedMs: number;
}

export const ATTRACT_LOOP_CHECKPOINTS: readonly AttractLoopCheckpointContract[] = Object.freeze([
  Object.freeze({ description: 'Title screen mid-fade after the DOOM logo finishes drawing', phase: 'title' as const, scheduledAtElapsedMs: 3_000 } satisfies AttractLoopCheckpointContract),
  Object.freeze({ description: 'DEMO1 attract playback after the title screen advances', phase: 'demo-one' as const, scheduledAtElapsedMs: 9_000 } satisfies AttractLoopCheckpointContract),
  Object.freeze({ description: 'DEMO1 sampled deeper into playback', phase: 'demo-one' as const, scheduledAtElapsedMs: 18_000 } satisfies AttractLoopCheckpointContract),
  Object.freeze({ description: 'Post-DEMO1 interstitial / credits / order info screen', phase: 'post-demo-one-interstitial' as const, scheduledAtElapsedMs: 30_000 } satisfies AttractLoopCheckpointContract),
] satisfies readonly AttractLoopCheckpointContract[]);

export const ATTRACT_LOOP_PHASES: readonly AttractLoopPhase[] = Object.freeze(['demo-one', 'demo-three', 'demo-two', 'post-demo-one-interstitial', 'post-demo-three-interstitial', 'post-demo-two-interstitial', 'title']);

export interface AttractLoopCheckpointEvidence {
...
export interface ReferenceIntermissionFinaleEvidence {
  readonly attractCheckpoints: readonly AttractLoopCheckpointEvidence[];
  ...
  readonly terminationCause: ReferenceIntermissionFinaleTerminationCause;
...
export interface CaptureReferenceIntermissionFinaleOverrides {
  readonly executableFilename?: string;
...
export class ReferenceIntermissionFinaleWindowNotFoundError extends Error {
  public constructor(windowTitle: string, timeoutMs: number) {
    ...
    this.name = 'ReferenceIntermissionFinaleWindowNotFoundError';
  }
}
...
export async function captureReferenceIntermissionFinale(overrides: CaptureReferenceIntermissionFinaleOverrides = {}): Promise<ReferenceIntermissionFinaleEvidence> {
  ...
  try {
    ...
    try {
      ...
      if (discovered === null) {
        throw new ReferenceIntermissionFinaleWindowNotFoundError(CHOCOLATE_DOOM_WINDOW_TITLE, findWindowTimeoutMs);
      }
      ...
      const attractCheckpoints: AttractLoopCheckpointEvidence[] = [];
      for (let checkpointIndex = 0; checkpointIndex < ATTRACT_LOOP_CHECKPOINTS.length; checkpointIndex += 1) {
        const checkpoint = ATTRACT_LOOP_CHECKPOINTS[checkpointIndex]!;
        while (nowMs() - startReference < checkpoint.scheduledAtElapsedMs) {
          await sleepMs(Math.max(1, checkpoint.scheduledAtElapsedMs - (nowMs() - startReference)));
        }
        user32Foreground.symbols.SetForegroundWindow(hWnd);
        ...
        const schedulingDriftMs = capturedAtElapsedMs - checkpoint.scheduledAtElapsedMs;
        attractCheckpoints.push(
          Object.freeze({
            capturedAtElapsedMs,
            checkpointIndex,
            ...
            scheduledAtElapsedMs: checkpoint.scheduledAtElapsedMs,
            schedulingDriftMs,
          } satisfies AttractLoopCheckpointEvidence),
        );
      }

      let killedAtElapsedMs = nowMs() - startReference;
      let terminationCause: ReferenceIntermissionFinaleTerminationCause = 'natural-exit';
      ...
      return Object.freeze({
        attractCheckpoints: Object.freeze([...attractCheckpoints]) as readonly AttractLoopCheckpointEvidence[],
        cleanShutdown,
        executableFilename,
        exitCode,
        exitedAtElapsedMs,
      ...
      } satisfies ReferenceIntermissionFinaleEvidence);
    } catch (caughtError) {
      if (subprocess.exitCode === null) {
        subprocess.kill();
      }
