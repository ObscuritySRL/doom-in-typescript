# [ ] STEP 05-002: Verify Lump Name Lookup Semantics
...
Verify Lump Name Lookup Semantics

## goal

Deliver the smallest verified increment for verify lump name lookup semantics while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- test/vanilla_parity/wad/verify-lump-name-lookup-semantics.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/wad/verify-lump-name-lookup-semantics.test.ts`
- `bun test`
...
- A committed and pushed change set for 05-002 with test/vanilla_parity/wad/verify-lump-name-lookup-semantics.test.ts proving the new behavior or audit result.
