# [ ] STEP 06-026: Compare E1M2 Map Setup State
...
Compare E1M2 Map Setup State

## goal

Deliver the smallest verified increment for compare e1m2 map setup state while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/map/compare-e1m2-map-setup-state.ts
- test/vanilla_parity/map/compare-e1m2-map-setup-state.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/map/compare-e1m2-map-setup-state.test.ts`
- `bun test`
...
- A committed and pushed change set for 06-026 with test/vanilla_parity/map/compare-e1m2-map-setup-state.test.ts proving the new behavior or audit result.
