# [ ] STEP 01-008: Inventory Player Modules
...
Inventory Player Modules

## goal

Deliver the smallest verified increment for inventory player modules while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- plan_vanilla_parity/current-state/inventory-player-modules.json
- test/vanilla_parity/current-state/inventory-player-modules.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/current-state/inventory-player-modules.test.ts`
- `bun test`
...
- A committed and pushed change set for 01-008 with test/vanilla_parity/current-state/inventory-player-modules.test.ts proving the new behavior or audit result.
