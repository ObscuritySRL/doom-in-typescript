# [ ] STEP 12-007: Persist Chat Macros
...
Deliver the smallest verified increment for persist chat macros while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/config/persist-chat-macros.ts
- test/vanilla_parity/save/persist-chat-macros.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/save/persist-chat-macros.test.ts`
- `bun test`
...
- A committed and pushed change set for 12-007 with test/vanilla_parity/save/persist-chat-macros.test.ts proving the new behavior or audit result.
