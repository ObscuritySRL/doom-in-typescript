# [ ] STEP 12-005: Persist Sound Settings
...
Deliver the smallest verified increment for persist sound settings while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/config/persist-sound-settings.ts
- test/vanilla_parity/save/persist-sound-settings.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/save/persist-sound-settings.test.ts`
- `bun test`
...
- A committed and pushed change set for 12-005 with test/vanilla_parity/save/persist-sound-settings.test.ts proving the new behavior or audit result.
