# [ ] STEP 12-027: Gate Config Compatibility
...
Gate Config Compatibility

## goal

Deliver the smallest verified increment for gate config compatibility while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/config/gate-config-compatibility.ts
- test/vanilla_parity/save/gate-config-compatibility.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/save/gate-config-compatibility.test.ts`
- `bun test`
...
- A committed and pushed change set for 12-027 with test/vanilla_parity/save/gate-config-compatibility.test.ts proving the new behavior or audit result.
