# [ ] STEP 12-014: Serialize Mobj State
...
Serialize Mobj State

## goal

Deliver the smallest verified increment for serialize mobj state while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/save/serialize-mobj-state.ts
- test/vanilla_parity/save/serialize-mobj-state.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/save/serialize-mobj-state.test.ts`
- `bun test`
...
- A committed and pushed change set for 12-014 with test/vanilla_parity/save/serialize-mobj-state.test.ts proving the new behavior or audit result.
