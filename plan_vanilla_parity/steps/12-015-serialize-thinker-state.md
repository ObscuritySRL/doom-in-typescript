# [ ] STEP 12-015: Serialize Thinker State
...
Serialize Thinker State

## goal

Deliver the smallest verified increment for serialize thinker state while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/save/serialize-thinker-state.ts
- test/vanilla_parity/save/serialize-thinker-state.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/save/serialize-thinker-state.test.ts`
- `bun test`
...
- A committed and pushed change set for 12-015 with test/vanilla_parity/save/serialize-thinker-state.test.ts proving the new behavior or audit result.
