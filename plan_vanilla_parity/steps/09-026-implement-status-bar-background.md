# [ ] STEP 09-026: Implement Status Bar Background
...
Implement Status Bar Background

## goal

Deliver the smallest verified increment for implement status bar background while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- test/vanilla_parity/render/implement-status-bar-background.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/render/implement-status-bar-background.test.ts`
- `bun test`
...
- A committed and pushed change set for 09-026 with test/vanilla_parity/render/implement-status-bar-background.test.ts proving the new behavior or audit result.
