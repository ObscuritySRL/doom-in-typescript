# [ ] STEP 03-001: Add Root Doom Ts Bun Entrypoint
...
Add Root Doom Ts Bun Entrypoint

## goal

Deliver the smallest verified increment for add root doom ts bun entrypoint while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- `bun run format`
- `bun test test/vanilla_parity/launch/add-root-doom-ts-bun-entrypoint.test.ts`
- `bun test`
...
- A committed and pushed change set for 03-001 with test/vanilla_parity/launch/add-root-doom-ts-bun-entrypoint.test.ts proving the new behavior or audit result.
