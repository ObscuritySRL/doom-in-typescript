# [ ] STEP 05-010: Parse Texture Two When Present
...
Parse Texture Two When Present

## goal

Deliver the smallest verified increment for parse texture two when present while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- test/vanilla_parity/wad/parse-texture-two-when-present.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/wad/parse-texture-two-when-present.test.ts`
- `bun test`
...
- A committed and pushed change set for 05-010 with test/vanilla_parity/wad/parse-texture-two-when-present.test.ts proving the new behavior or audit result.
