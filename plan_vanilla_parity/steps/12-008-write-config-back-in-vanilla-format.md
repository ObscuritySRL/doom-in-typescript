# [ ] STEP 12-008: Write Config Back In Vanilla Format
...
Write Config Back In Vanilla Format

## goal

Deliver the smallest verified increment for write config back in vanilla format while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/config/write-config-back-in-vanilla-format.ts
- test/vanilla_parity/save/write-config-back-in-vanilla-format.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/save/write-config-back-in-vanilla-format.test.ts`
- `bun test`
...
- A committed and pushed change set for 12-008 with test/vanilla_parity/save/write-config-back-in-vanilla-format.test.ts proving the new behavior or audit result.
