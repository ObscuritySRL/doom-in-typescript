# [ ] STEP 03-022: Implement Gameplay Key Mapping From Config
...
Implement Gameplay Key Mapping From Config

## goal

Deliver the smallest verified increment for implement gameplay key mapping from config while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- `bun run format`
- `bun test test/vanilla_parity/launch/implement-gameplay-key-mapping-from-config.test.ts`
- `bun test`
...
- A committed and pushed change set for 03-022 with test/vanilla_parity/launch/implement-gameplay-key-mapping-from-config.test.ts proving the new behavior or audit result.
