# [ ] STEP 03-021: Implement Menu Key Mapping From Config
...
Implement Menu Key Mapping From Config

## goal

Deliver the smallest verified increment for implement menu key mapping from config while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- `bun run format`
- `bun test test/vanilla_parity/launch/implement-menu-key-mapping-from-config.test.ts`
- `bun test`
...
- A committed and pushed change set for 03-021 with test/vanilla_parity/launch/implement-menu-key-mapping-from-config.test.ts proving the new behavior or audit result.
