# [ ] STEP 06-018: Implement Reject Sight Fast Path
...
Implement Reject Sight Fast Path

## goal

Deliver the smallest verified increment for implement reject sight fast path while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/map/implement-reject-sight-fast-path.ts
- test/vanilla_parity/map/implement-reject-sight-fast-path.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/map/implement-reject-sight-fast-path.test.ts`
- `bun test`
...
- A committed and pushed change set for 06-018 with test/vanilla_parity/map/implement-reject-sight-fast-path.test.ts proving the new behavior or audit result.
