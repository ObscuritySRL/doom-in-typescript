# [ ] STEP 04-028: Add Determinism State Hash Hook
...
Add Determinism State Hash Hook

## goal

Deliver the smallest verified increment for add determinism state hash hook while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- `bun run format`
- `bun test test/vanilla_parity/core/add-determinism-state-hash-hook.test.ts`
- `bun test`
...
- A committed and pushed change set for 04-028 with test/vanilla_parity/core/add-determinism-state-hash-hook.test.ts proving the new behavior or audit result.
