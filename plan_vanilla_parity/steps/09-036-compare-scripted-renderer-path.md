# [ ] STEP 09-036: Compare Scripted Renderer Path
...
Compare Scripted Renderer Path

## goal

Deliver the smallest verified increment for compare scripted renderer path while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/render/compare-scripted-renderer-path.ts
- test/vanilla_parity/render/compare-scripted-renderer-path.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/render/compare-scripted-renderer-path.test.ts`
- `bun test`
...
- A committed and pushed change set for 09-036 with test/vanilla_parity/render/compare-scripted-renderer-path.test.ts proving the new behavior or audit result.
