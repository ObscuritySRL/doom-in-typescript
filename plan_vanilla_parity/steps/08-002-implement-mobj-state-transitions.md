# [ ] STEP 08-002: Implement Mobj State Transitions
...
Implement Mobj State Transitions

## goal

Deliver the smallest verified increment for implement mobj state transitions while preserving vanilla DOOM 1.9 behavior and avoiding unverified assumptions.
...
- src/ai/implement-mobj-state-transitions.ts
- test/vanilla_parity/ai/implement-mobj-state-transitions.test.ts
...
- `bun run format`
- `bun test test/vanilla_parity/ai/implement-mobj-state-transitions.test.ts`
- `bun test`
...
- A committed and pushed change set for 08-002 with test/vanilla_parity/ai/implement-mobj-state-transitions.test.ts proving the new behavior or audit result.
