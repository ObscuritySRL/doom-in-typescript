...
function Test-ClaudeCommand {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            return "Claude Code command failed version check: $($output | Out-String)"
        }
    }
    catch {
        return "Claude Code command failed version check: $($_.Exception.Message)"
    }
...
function Test-LimitReached {
    ...
    return $Text -match "(?im)\b(rate|usage)\s+limit\b" -or $Text -match "(?im)\blimit\s+reached\b" -or $Text -match "(?im)You('ve| have)? hit your limit"
}
...
if ($MaxIterations -eq 0) {
    Write-Host "Validated prompt path, working directory, log directory, and lane lock directory. MaxIterations is 0, so no Claude Code invocation was made."
    exit 0
}

$owner = "$env:USERNAME@$env:COMPUTERNAME pid=$PID agent=Claude Code model=$Model"
$acquireArguments = @(
    "acquire",
    "--plan-directory", $PlanDirectory,
    "--lock-directory", $LaneLockDirectory,
...
try {
    ...
    $resolvedClaudeCommand = Resolve-ClaudeCommand -Value $ClaudeCommand
    if (-not $resolvedClaudeCommand) {
        Write-LoopSummary -Status "CLI_ERROR" -Reason "Claude Code command not found: $ClaudeCommand. Install Claude Code and restart PowerShell so claude is on PATH, or pass -ClaudeCommand with the full CLI path."
        exit 2
    }

    $claudeCommandError = Test-ClaudeCommand -Value $resolvedClaudeCommand
    if ($claudeCommandError) {
        Write-LoopSummary -Status "CLI_ERROR" -Reason "$claudeCommandError Install Claude Code and restart PowerShell, or pass -ClaudeCommand with the full CLI path."
        exit 2
    }
    ...
    try {
        ...
        while ($iteration -le $MaxIterations) {
            ...
            $responsePath = Join-Path $LogDirectory ("loop_claude_{0}_{1:D4}_{2}.txt" -f $script:SelectedLane, $iteration, $timestamp)
            ...
            Write-Host "Claude command: $resolvedClaudeCommand"
            ...
            if (Test-LimitReached -Text $response) {
                Write-LoopSummary -Status "LIMIT_REACHED" -Reason "Claude Code reported a rate or usage limit." -ResponsePath $responsePath
                exit 3
            }

            if ($exitCode -ne 0 -and -not $response.Trim()) {
                Write-LoopSummary -Status "CLI_ERROR" -Reason "Claude Code exited with code $exitCode and produced no response body." -ResponsePath $responsePath
                $iteration++
            ...
            if (-not $status) {
                if ($exitCode -ne 0) {
                    Write-LoopSummary -Status "CLI_ERROR" -Reason "Claude Code exited with code $exitCode and did not emit RLP_STATUS." -ResponsePath $responsePath
                    $iteration++
                    if ($SleepSeconds -gt 0) {
                        Start-Sleep -Seconds $SleepSeconds
                    }
                ...
                Write-LoopSummary -Status "UNKNOWN_STATUS" -Reason "Claude Code response omitted or malformed RLP_STATUS." -ResponsePath $responsePath
                $iteration++
                if ($SleepSeconds -gt 0) {
                    Start-Sleep -Seconds $SleepSeconds
                }
            ...
            switch ($status) {
                ...
                default {
                    Write-LoopSummary -Status "UNKNOWN_STATUS" -Reason "Claude Code returned an unrecognized RLP_STATUS value: $status" -StepId $stepId -StepTitle $stepTitle -ResponsePath $responsePath
                }
            }
