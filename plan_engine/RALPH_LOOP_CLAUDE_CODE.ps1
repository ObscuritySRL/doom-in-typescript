param(
    [string]$PromptPath = "D:\Projects\bun-win32\PROMPT.md",
    [string]$PrePromptPath = "D:\Projects\bun-win32\doom_codex\plans\PRE_PROMPT.md",
    [string]$WorkingDirectory = "D:\Projects\bun-win32\doom_codex",
    [string]$LogDirectory = "D:\Projects\bun-win32\doom_codex\plans\loop_logs",
...
try {
    ...
    while ($iteration -le $MaxIterations) {
        ...
        if (Test-LimitReached -Text $preResponse) {
            Write-LoopSummary -Status "LIMIT_REACHED" -Reason "Claude reported a rate or usage limit during audit pass." -ResponsePath $prePath
            exit 3
        }

        if ($preExitCode -ne 0 -and -not $preResponse.Trim()) {
            Write-LoopSummary -Status "CLI_ERROR" -Reason "Claude audit pass exited with code $preExitCode and produced no response body." -ResponsePath $prePath
            $iteration++
