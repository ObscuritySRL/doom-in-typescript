# [x] STEP 10-001: player-spawn-and-reset

## Goal

Implement player spawn and reset so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\player\playerSpawn.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\player\player-spawn.test.ts

## Verification

- `bun test test/player/player-spawn.test.ts`
- `bun test`
...
- The targeted test file `test/player/player-spawn.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
