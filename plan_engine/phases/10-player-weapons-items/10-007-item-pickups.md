# [ ] STEP 10-007: item-pickups

## Goal

Implement item pickups so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\player\pickups.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\player\pickups.test.ts

## Verification

- `bun test test/player/pickups.test.ts`
- `bun test`
...
- The targeted test file `test/player/pickups.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
