# [x] STEP 10-004: weapon-state-machine

## Goal

Implement weapon state machine so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\player\weaponStates.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\player\weapon-states.test.ts

## Verification

- `bun test test/player/weapon-states.test.ts`
- `bun test`
...
- The targeted test file `test/player/weapon-states.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
