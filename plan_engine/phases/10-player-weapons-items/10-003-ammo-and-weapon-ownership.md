# [x] STEP 10-003: ammo-and-weapon-ownership

## Goal

Implement ammo and weapon ownership so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\player\weapons.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\player\weapon-ownership.test.ts

## Verification

- `bun test test/player/weapon-ownership.test.ts`
- `bun test`
...
- The targeted test file `test/player/weapon-ownership.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
