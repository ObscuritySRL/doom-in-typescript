# [ ] STEP 10-008: powerup-timers-and-palette-effects

## Goal

Implement powerup timers and palette effects so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\player\powerups.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\player\powerups.test.ts

## Verification

- `bun test test/player/powerups.test.ts`
- `bun test`
...
- The targeted test file `test/player/powerups.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
