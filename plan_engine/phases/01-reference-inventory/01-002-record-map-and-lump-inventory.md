# [ ] STEP 01-002: record-map-and-lump-inventory

## Goal

Implement record map and lump inventory so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\reference\manifests\wad-map-summary.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\reference\wad-map-summary.test.ts

## Verification

- `bun test test/reference/wad-map-summary.test.ts`
- `bun test`
...
- The targeted test file `test/reference/wad-map-summary.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
