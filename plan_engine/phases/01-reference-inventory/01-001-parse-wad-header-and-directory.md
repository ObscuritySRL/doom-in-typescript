# [ ] STEP 01-001: parse-wad-header-and-directory

## Goal

Implement parse wad header and directory so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\wad\header.ts
- d:\Projects\bun-win32\doom_codex\src\wad\directory.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\header-directory.test.ts

## Verification

- `bun test test/wad/header-directory.test.ts`
- `bun test`
...
- The targeted test file `test/wad/header-directory.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
