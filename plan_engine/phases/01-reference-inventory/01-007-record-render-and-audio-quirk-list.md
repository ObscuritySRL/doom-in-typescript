# [ ] STEP 01-007: record-render-and-audio-quirk-list

## Goal

Implement record render and audio quirk list so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\reference\manifests\quirk-manifest.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\reference\quirk-manifest.test.ts

## Verification

- `bun test test/reference/quirk-manifest.test.ts`
- `bun test`
...
- The targeted test file `test/reference/quirk-manifest.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
