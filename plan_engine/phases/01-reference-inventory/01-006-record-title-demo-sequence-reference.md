# [ ] STEP 01-006: record-title-demo-sequence-reference

## Goal

Implement record title demo sequence reference so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\reference\manifests\title-sequence.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\reference\title-sequence.test.ts

## Verification

- `bun test test/reference/title-sequence.test.ts`
- `bun test`
...
- The targeted test file `test/reference/title-sequence.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
