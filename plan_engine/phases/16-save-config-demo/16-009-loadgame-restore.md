# [ ] STEP 16-009: loadgame-restore

## Goal

Implement loadgame restore so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\save\loadgame.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\save\loadgame.test.ts

## Verification

- `bun test test/save/loadgame.test.ts`
- `bun test`
...
- The targeted test file `test/save/loadgame.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
