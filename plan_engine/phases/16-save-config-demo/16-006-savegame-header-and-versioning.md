# [ ] STEP 16-006: savegame-header-and-versioning

## Goal

Implement savegame header and versioning so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\save\saveHeader.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\save\save-header.test.ts

## Verification

- `bun test test/save/save-header.test.ts`
- `bun test`
...
- The targeted test file `test/save/save-header.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
