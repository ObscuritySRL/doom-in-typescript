# [x] STEP 16-001: default-cfg-parser

## Goal

Implement default cfg parser so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\config\defaultCfg.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\config\default-cfg-parse.test.ts

## Verification

- `bun test test/config/default-cfg-parse.test.ts`
- `bun test`
...
- The targeted test file `test/config/default-cfg-parse.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
