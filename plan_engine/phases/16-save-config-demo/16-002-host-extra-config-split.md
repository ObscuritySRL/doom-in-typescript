# [ ] STEP 16-002: host-extra-config-split

## Goal

Implement host extra config split so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\config\hostConfig.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\config\host-config.test.ts

## Verification

- `bun test test/config/host-config.test.ts`
- `bun test`
...
- The targeted test file `test/config/host-config.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
