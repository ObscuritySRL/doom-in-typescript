# [ ] STEP 16-010: vanilla-limit-enforcement

## Goal

Implement vanilla limit enforcement so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\save\vanillaLimits.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\save\vanilla-limits.test.ts

## Verification

- `bun test test/save/vanilla-limits.test.ts`
- `bun test`
...
- The targeted test file `test/save/vanilla-limits.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
