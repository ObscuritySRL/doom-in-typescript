# [x] STEP 12-004: floor-movers

## Goal

Implement floor movers so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\specials\floors.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\specials\floors.test.ts

## Verification

- `bun test test/specials/floors.test.ts`
- `bun test`
...
- The targeted test file `test/specials/floors.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
