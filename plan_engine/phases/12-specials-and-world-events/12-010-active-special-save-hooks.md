# [ ] STEP 12-010: active-special-save-hooks

## Goal

Implement active special save hooks so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\specials\activeSpecials.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\specials\active-specials.test.ts

## Verification

- `bun test test/specials/active-specials.test.ts`
- `bun test`
...
- The targeted test file `test/specials/active-specials.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
