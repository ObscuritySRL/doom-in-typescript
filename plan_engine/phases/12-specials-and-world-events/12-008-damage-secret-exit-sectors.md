# [ ] STEP 12-008: damage-secret-exit-sectors

## Goal

Implement damage secret exit sectors so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\specials\sectorSpecials.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\specials\sector-specials.test.ts

## Verification

- `bun test test/specials/sector-specials.test.ts`
- `bun test`
...
- The targeted test file `test/specials/sector-specials.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
