# [ ] STEP 12-002: door-specials

## Goal

Implement door specials so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\specials\doors.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\specials\doors.test.ts

## Verification

- `bun test test/specials/doors.test.ts`
- `bun test`
...
- The targeted test file `test/specials/doors.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
