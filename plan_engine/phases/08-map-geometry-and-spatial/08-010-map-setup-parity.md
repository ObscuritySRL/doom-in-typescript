# [ ] STEP 08-010: map-setup-parity

## Goal

Implement map setup parity so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\map\mapSetup.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\map\map-setup-parity.test.ts

## Verification

- `bun test test/map/map-setup-parity.test.ts`
- `bun test`
...
- The targeted test file `test/map/map-setup-parity.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
