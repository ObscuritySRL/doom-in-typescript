# [ ] STEP 08-002: linedefs-sidedefs-sectors-vertexes-loaders

## Goal

Implement linedefs sidedefs sectors vertexes loaders so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\map\lineSectorGeometry.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\map\struct-loaders.test.ts

## Verification

- `bun test test/map/struct-loaders.test.ts`
- `bun test`
...
- The targeted test file `test/map/struct-loaders.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
