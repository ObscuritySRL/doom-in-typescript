# [x] STEP 08-008: blockmap-iteration-order

## Goal

Implement blockmap iteration order so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\map\blockmapIter.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\map\blockmap-iteration.test.ts

## Verification

- `bun test test/map/blockmap-iteration.test.ts`
- `bun test`
...
- The targeted test file `test/map/blockmap-iteration.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
