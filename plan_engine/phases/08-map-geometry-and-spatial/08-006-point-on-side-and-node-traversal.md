# [ ] STEP 08-006: point-on-side-and-node-traversal

## Goal

Implement point on side and node traversal so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\map\nodeTraversal.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\map\node-traversal.test.ts

## Verification

- `bun test test/map/node-traversal.test.ts`
- `bun test`
...
- The targeted test file `test/map/node-traversal.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
