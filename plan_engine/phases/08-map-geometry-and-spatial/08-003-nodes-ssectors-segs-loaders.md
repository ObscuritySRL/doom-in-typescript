# [x] STEP 08-003: nodes-ssectors-segs-loaders

## Goal

Implement nodes ssectors segs loaders so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\map\bspStructs.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\map\bsp-struct-loaders.test.ts

## Verification

- `bun test test/map/bsp-struct-loaders.test.ts`
- `bun test`
...
- The targeted test file `test/map/bsp-struct-loaders.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
