# [ ] STEP 02-001: define-reference-sandbox-copy-policy

## Goal

Implement define reference sandbox copy policy so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\oracles\referenceSandbox.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\oracles\reference-sandbox-policy.test.ts

## Verification

- `bun test test/oracles/reference-sandbox-policy.test.ts`
- `bun test`
...
- The targeted test file `test/oracles/reference-sandbox-policy.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
