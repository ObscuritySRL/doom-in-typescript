# [ ] STEP 02-010: define-manual-a-b-gates

## Goal

Implement define manual a b gates so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\oracles\manualGatePolicy.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\oracles\manual-gates.test.ts

## Verification

- `bun test test/oracles/manual-gates.test.ts`
- `bun test`
...
- The targeted test file `test/oracles/manual-gates.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
