# [ ] STEP 02-002: define-reference-run-manifest

## Goal

Implement define reference run manifest so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\oracles\referenceRunManifest.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\oracles\reference-run-manifest.test.ts

## Verification

- `bun test test/oracles/reference-run-manifest.test.ts`
- `bun test`
...
- The targeted test file `test/oracles/reference-run-manifest.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
