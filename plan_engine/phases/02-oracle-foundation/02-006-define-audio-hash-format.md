# [ ] STEP 02-006: define-audio-hash-format

## Goal

Implement define audio hash format so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\oracles\audioHash.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\oracles\audio-hash-format.test.ts

## Verification

- `bun test test/oracles/audio-hash-format.test.ts`
- `bun test`
...
- The targeted test file `test/oracles/audio-hash-format.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
