# [ ] STEP 02-003: define-input-script-format

## Goal

Implement define input script format so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\oracles\inputScript.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\oracles\input-script-format.test.ts

## Verification

- `bun test test/oracles/input-script-format.test.ts`
- `bun test`
...
- The targeted test file `test/oracles/input-script-format.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
