# [ ] STEP 02-004: define-state-hash-format

## Goal

Implement define state hash format so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\oracles\stateHash.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\oracles\state-hash-format.test.ts

## Verification

- `bun test test/oracles/state-hash-format.test.ts`
- `bun test`
...
- The targeted test file `test/oracles/state-hash-format.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
