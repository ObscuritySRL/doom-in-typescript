# [ ] STEP 02-009: prove-reference-run-isolation

## Goal

Implement prove reference run isolation so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\tools\reference\isolationProbe.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\oracles\reference-run-isolation.test.ts

## Verification

- `bun test test/oracles/reference-run-isolation.test.ts`
- `bun test`
...
- The targeted test file `test/oracles/reference-run-isolation.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
