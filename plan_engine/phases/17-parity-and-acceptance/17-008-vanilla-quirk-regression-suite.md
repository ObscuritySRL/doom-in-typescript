# [ ] STEP 17-008: vanilla-quirk-regression-suite

## Goal

Implement vanilla quirk regression suite so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test\parity\fixtures\quirkCases.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\parity\quirk-regressions.test.ts

## Verification

- `bun test test/parity/quirk-regressions.test.ts`
- `bun test`
...
- The targeted test file `test/parity/quirk-regressions.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
