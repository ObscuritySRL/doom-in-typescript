# [ ] STEP 17-009: full-shareware-e1-acceptance

## Goal

Implement full shareware e1 acceptance so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test\parity\fixtures\e1Acceptance.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\parity\full-e1-acceptance.test.ts

## Verification

- `bun test test/parity/full-e1-acceptance.test.ts`
- `bun test`
...
- The targeted test file `test/parity/full-e1-acceptance.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
