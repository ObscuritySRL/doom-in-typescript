# [ ] STEP 17-004: framebuffer-crc-suite

## Goal

Implement framebuffer crc suite so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test\parity\fixtures\framebufferHashes.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\parity\framebuffer-crc.test.ts

## Verification

- `bun test test/parity/framebuffer-crc.test.ts`
- `bun test`
...
- The targeted test file `test/parity/framebuffer-crc.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
