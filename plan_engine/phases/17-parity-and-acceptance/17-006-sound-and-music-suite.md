# [ ] STEP 17-006: sound-and-music-suite

## Goal

Implement sound and music suite so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test\parity\fixtures\audioHashes.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\parity\audio-suite.test.ts

## Verification

- `bun test test/parity/audio-suite.test.ts`
- `bun test`
...
- The targeted test file `test/parity/audio-suite.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
