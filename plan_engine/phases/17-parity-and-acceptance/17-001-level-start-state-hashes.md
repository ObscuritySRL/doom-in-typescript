# [ ] STEP 17-001: level-start-state-hashes

## Goal

Implement level start state hashes so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test\parity\fixtures\levelStartHashes.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\parity\level-start-state-hash.test.ts

## Verification

- `bun test test/parity/level-start-state-hash.test.ts`
- `bun test`
...
- The targeted test file `test/parity/level-start-state-hash.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
