# [ ] STEP 15-005: sound-start-stop-update-order

## Goal

Implement sound start stop update order so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\audio\soundSystem.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\audio\sound-order.test.ts

## Verification

- `bun test test/audio/sound-order.test.ts`
- `bun test`
...
- The targeted test file `test/audio/sound-order.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
