# [ ] STEP 15-002: eight-channel-allocator

## Goal

Implement eight channel allocator so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\audio\channels.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\audio\channel-allocation.test.ts

## Verification

- `bun test test/audio/channel-allocation.test.ts`
- `bun test`
...
- The targeted test file `test/audio/channel-allocation.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
