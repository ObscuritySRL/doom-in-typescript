# [ ] STEP 15-007: mus-parser

## Goal

Implement mus parser so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\audio\musParser.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\audio\mus-parser.test.ts

## Verification

- `bun test test/audio/mus-parser.test.ts`
- `bun test`
...
- The targeted test file `test/audio/mus-parser.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
