# [x] STEP 15-009: opl-register-model

## Goal

Implement opl register model so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\audio\oplRegisters.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\audio\opl-registers.test.ts

## Verification

- `bun test test/audio/opl-registers.test.ts`
- `bun test`
...
- The targeted test file `test/audio/opl-registers.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
