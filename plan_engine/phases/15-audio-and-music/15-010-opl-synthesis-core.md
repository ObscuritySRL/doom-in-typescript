# [ ] STEP 15-010: opl-synthesis-core

## Goal

Implement opl synthesis core so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\audio\oplSynth.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\audio\opl-synth.test.ts

## Verification

- `bun test test/audio/opl-synth.test.ts`
- `bun test`
...
- The targeted test file `test/audio/opl-synth.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
