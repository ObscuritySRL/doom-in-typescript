# [ ] STEP 15-004: sound-origin-updates

## Goal

Implement sound origin updates so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\audio\soundOrigins.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\audio\sound-origins.test.ts

## Verification

- `bun test test/audio/sound-origins.test.ts`
- `bun test`
...
- The targeted test file `test/audio/sound-origins.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
