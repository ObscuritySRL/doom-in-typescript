# [ ] STEP 15-006: pcm-mixer-clipping-and-stepping

## Goal

Implement pcm mixer clipping and stepping so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\audio\pcmMixer.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\audio\pcm-mixer.test.ts

## Verification

- `bun test test/audio/pcm-mixer.test.ts`
- `bun test`
...
- The targeted test file `test/audio/pcm-mixer.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
