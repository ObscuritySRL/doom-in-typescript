# [ ] STEP 11-007: pain-death-raise-explode

## Goal

Implement pain death raise explode so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\ai\stateTransitions.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\ai\state-transitions.test.ts

## Verification

- `bun test test/ai/state-transitions.test.ts`
- `bun test`
...
- The targeted test file `test/ai/state-transitions.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
