# [ ] STEP 11-001: recursive-sound-propagation

## Goal

Implement recursive sound propagation so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\ai\soundPropagation.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\ai\sound-propagation.test.ts

## Verification

- `bun test test/ai/sound-propagation.test.ts`
- `bun test`
...
- The targeted test file `test/ai/sound-propagation.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
