# [ ] STEP 11-002: target-acquisition-and-sight

## Goal

Implement target acquisition and sight so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\ai\targeting.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\ai\target-acquisition.test.ts

## Verification

- `bun test test/ai/target-acquisition.test.ts`
- `bun test`
...
- The targeted test file `test/ai/target-acquisition.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
