# [ ] STEP 11-005: missile-range-checks

## Goal

Implement missile range checks so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\ai\missileRange.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\ai\missile-range.test.ts

## Verification

- `bun test test/ai/missile-range.test.ts`
- `bun test`
...
- The targeted test file `test/ai/missile-range.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
