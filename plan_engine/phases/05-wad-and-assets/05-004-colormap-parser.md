# [x] STEP 05-004: colormap-parser

## Goal

Implement colormap parser so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\assets\colormap.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\colormap.test.ts

## Verification

- `bun test test/wad/colormap.test.ts`
- `bun test`
...
- The targeted test file `test/wad/colormap.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
