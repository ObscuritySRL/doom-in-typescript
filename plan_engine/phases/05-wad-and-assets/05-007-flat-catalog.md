# [x] STEP 05-007: flat-catalog

## Goal

Implement flat catalog so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\assets\flats.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\flats.test.ts

## Verification

- `bun test test/wad/flats.test.ts`
- `bun test`
...
- The targeted test file `test/wad/flats.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
