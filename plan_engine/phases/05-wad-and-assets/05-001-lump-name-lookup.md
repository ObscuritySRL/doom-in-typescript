# [ ] STEP 05-001: lump-name-lookup

## Goal

Implement lump name lookup so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\wad\lumpLookup.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\lump-lookup.test.ts

## Verification

- `bun test test/wad/lump-lookup.test.ts`
- `bun test`
...
- The targeted test file `test/wad/lump-lookup.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
