# [x] STEP 05-006: texture1-parser

## Goal

Implement texture1 parser so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\assets\texture1.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\texture1.test.ts

## Verification

- `bun test test/wad/texture1.test.ts`
- `bun test`
...
- The targeted test file `test/wad/texture1.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
