# [x] STEP 05-005: pnames-parser

## Goal

Implement pnames parser so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\assets\pnames.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\pnames.test.ts

## Verification

- `bun test test/wad/pnames.test.ts`
- `bun test`
...
- The targeted test file `test/wad/pnames.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
