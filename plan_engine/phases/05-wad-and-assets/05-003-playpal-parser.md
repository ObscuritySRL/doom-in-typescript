# [x] STEP 05-003: playpal-parser

## Goal

Implement playpal parser so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\assets\playpal.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\playpal.test.ts

## Verification

- `bun test test/wad/playpal.test.ts`
- `bun test`
...
- The targeted test file `test/wad/playpal.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
