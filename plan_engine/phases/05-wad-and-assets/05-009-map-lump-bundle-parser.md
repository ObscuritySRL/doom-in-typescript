# [ ] STEP 05-009: map-lump-bundle-parser

## Goal

Implement map lump bundle parser so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\map\mapBundle.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\map-bundle.test.ts

## Verification

- `bun test test/wad/map-bundle.test.ts`
- `bun test`
...
- The targeted test file `test/wad/map-bundle.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
