# [ ] STEP 05-002: marker-range-resolution

## Goal

Implement marker range resolution so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\wad\markerRange.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\wad\marker-range.test.ts

## Verification

- `bun test test/wad/marker-range.test.ts`
- `bun test`
...
- The targeted test file `test/wad/marker-range.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
