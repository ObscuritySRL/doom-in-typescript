# [ ] STEP 07-005: title-and-demo-loop-state-machine

## Goal

Implement title and demo loop state machine so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- https://github.com/chocolate-doom/chocolate-doom/blob/master/src/doom/d_main.c
...
- d:\Projects\bun-win32\doom_codex\src\bootstrap\titleLoop.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\bootstrap\title-loop.test.ts

## Verification

- `bun test test/bootstrap/title-loop.test.ts`
- `bun test`
...
- The targeted test file `test/bootstrap/title-loop.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
