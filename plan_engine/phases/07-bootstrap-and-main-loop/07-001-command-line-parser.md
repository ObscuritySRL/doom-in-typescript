# [x] STEP 07-001: command-line-parser

## Goal

Implement command line parser so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\bootstrap\cmdline.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\bootstrap\cmdline.test.ts

## Verification

- `bun test test/bootstrap/cmdline.test.ts`
- `bun test`
...
- The targeted test file `test/bootstrap/cmdline.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
