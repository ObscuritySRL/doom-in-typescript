# [x] STEP 07-004: d-doommain-init-order

## Goal

Implement d doommain init order so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\bootstrap\initOrder.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\bootstrap\init-order.test.ts

## Verification

- `bun test test/bootstrap/init-order.test.ts`
- `bun test`
...
- The targeted test file `test/bootstrap/init-order.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
