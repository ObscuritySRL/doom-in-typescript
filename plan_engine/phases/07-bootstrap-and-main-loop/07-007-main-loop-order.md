# [ ] STEP 07-007: main-loop-order

## Goal

Implement main loop order so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\mainLoop.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\bootstrap\main-loop-order.test.ts

## Verification

- `bun test test/bootstrap/main-loop-order.test.ts`
- `bun test`
...
- The targeted test file `test/bootstrap/main-loop-order.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
