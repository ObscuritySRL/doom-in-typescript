# [ ] STEP 07-002: config-load-precedence

## Goal

Implement config load precedence so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\bootstrap\config.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\bootstrap\config-precedence.test.ts

## Verification

- `bun test test/bootstrap/config-precedence.test.ts`
- `bun test`
...
- The targeted test file `test/bootstrap/config-precedence.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
