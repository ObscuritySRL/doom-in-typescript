# [ ] STEP 14-003: hud-messages

## Goal

Implement hud messages so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\ui\hudMessages.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\ui\hud-messages.test.ts

## Verification

- `bun test test/ui/hud-messages.test.ts`
- `bun test`
...
- The targeted test file `test/ui/hud-messages.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
