...
Implement automap so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\ui\automap.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\ui\automap.test.ts

## Verification

- `bun test test/ui/automap.test.ts`
- `bun test`
...
- The targeted test file `test/ui/automap.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
