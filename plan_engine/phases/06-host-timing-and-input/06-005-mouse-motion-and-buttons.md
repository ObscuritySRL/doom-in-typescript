# [ ] STEP 06-005: mouse-motion-and-buttons

## Goal

Implement mouse motion and buttons so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\input\mouse.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\input\mouse-sampling.test.ts

## Verification

- `bun test test/input/mouse-sampling.test.ts`
- `bun test`
...
- The targeted test file `test/input/mouse-sampling.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
