# [x] STEP 06-003: message-pump-ordering

## Goal

Implement message pump ordering so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\host\win32\messagePump.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\host\message-pump.test.ts

## Verification

- `bun test test/host/message-pump.test.ts`
- `bun test`
...
- The targeted test file `test/host/message-pump.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
