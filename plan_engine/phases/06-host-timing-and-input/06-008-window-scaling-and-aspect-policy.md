# [ ] STEP 06-008: window-scaling-and-aspect-policy

## Goal

Implement window scaling and aspect policy so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\host\windowPolicy.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\host\window-policy.test.ts

## Verification

- `bun test test/host/window-policy.test.ts`
- `bun test`
...
- The targeted test file `test/host/window-policy.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
