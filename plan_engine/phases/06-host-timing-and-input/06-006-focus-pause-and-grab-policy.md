# [x] STEP 06-006: focus-pause-and-grab-policy

## Goal

Implement focus pause and grab policy so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\input\focusPolicy.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\input\focus-policy.test.ts

## Verification

- `bun test test/input/focus-policy.test.ts`
- `bun test`
...
- The targeted test file `test/input/focus-policy.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
