# [x] STEP 06-002: exact-35hz-tic-accumulator

## Goal

Implement exact 35hz tic accumulator so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\host\ticAccumulator.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\host\tic-accumulator.test.ts

## Verification

- `bun test test/host/tic-accumulator.test.ts`
- `bun test`
...
- The targeted test file `test/host/tic-accumulator.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
