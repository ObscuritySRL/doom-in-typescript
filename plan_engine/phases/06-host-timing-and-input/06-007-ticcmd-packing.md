# [ ] STEP 06-007: ticcmd-packing

## Goal

Implement ticcmd packing so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\input\ticcmd.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\input\ticcmd.test.ts

## Verification

- `bun test test/input/ticcmd.test.ts`
- `bun test`
...
- The targeted test file `test/input/ticcmd.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
