# [ ] STEP 06-004: keyboard-scan-code-mapping

## Goal

Implement keyboard scan code mapping so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\input\keyboard.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\input\keyboard-mapping.test.ts

## Verification

- `bun test test/input/keyboard-mapping.test.ts`
- `bun test`
...
- The targeted test file `test/input/keyboard-mapping.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
