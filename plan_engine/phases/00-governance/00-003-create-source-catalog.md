# [ ] STEP 00-003: create-source-catalog

## Goal

Implement create source catalog so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\reference\manifests\source-catalog.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\reference\source-catalog.test.ts

## Verification

- `bun test test/reference/source-catalog.test.ts`
- `bun test`
...
- The targeted test file `test/reference/source-catalog.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
