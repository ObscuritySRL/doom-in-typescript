# [x] STEP 00-005: define-oracle-schemas

## Goal

Implement define oracle schemas so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\oracles\schema.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\oracles\oracle-schema.test.ts

## Verification

- `bun test test/oracles/oracle-schema.test.ts`
- `bun test`
...
- The targeted test file `test/oracles/oracle-schema.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
