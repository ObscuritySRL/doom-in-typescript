# [x] STEP 00-002: record-reference-hashes

## Goal

Implement record reference hashes so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\reference\manifests\file-hashes.json

## Test Files

- d:\Projects\bun-win32\doom_codex\test\reference\hash-manifest.test.ts

## Verification

- `bun test test/reference/hash-manifest.test.ts`
- `bun test`
...
- The targeted test file `test/reference/hash-manifest.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
