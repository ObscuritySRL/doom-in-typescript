# [ ] STEP 04-002: fixed-add-sub-overflow

## Goal

Implement fixed add sub overflow so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test\core\fixed.add-sub.test.ts

## Verification

- `bun test test/core/fixed.add-sub.test.ts`
- `bun test`
...
- The targeted test file `test/core/fixed.add-sub.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
