# [ ] STEP 04-006: trig-lookups

## Goal

Implement trig lookups so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\core\trig.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\core\trig-lookup.test.ts

## Verification

- `bun test test/core/trig-lookup.test.ts`
- `bun test`
...
- The targeted test file `test/core/trig-lookup.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
