# [x] STEP 04-001: fixed-constants-and-type

## Goal

Implement fixed constants and type so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\core\fixed.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\core\fixed.constants.test.ts

## Verification

- `bun test test/core/fixed.constants.test.ts`
- `bun test`
...
- The targeted test file `test/core/fixed.constants.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
