# [ ] STEP 04-003: fixed-multiply

## Goal

Implement fixed multiply so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test\core\fixed.multiply.test.ts

## Verification

- `bun test test/core/fixed.multiply.test.ts`
- `bun test`
...
- The targeted test file `test/core/fixed.multiply.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
