# [ ] STEP 04-004: fixed-divide

## Goal

Implement fixed divide so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test\core\fixed.divide.test.ts

## Verification

- `bun test test/core/fixed.divide.test.ts`
- `bun test`
...
- The targeted test file `test/core/fixed.divide.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
