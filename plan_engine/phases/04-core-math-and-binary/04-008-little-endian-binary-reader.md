# [ ] STEP 04-008: little-endian-binary-reader

## Goal

Implement little endian binary reader so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\core\binaryReader.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\core\binary-reader.test.ts

## Verification

- `bun test test/core/binary-reader.test.ts`
- `bun test`
...
- The targeted test file `test/core/binary-reader.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
