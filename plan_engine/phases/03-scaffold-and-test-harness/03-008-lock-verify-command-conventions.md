# [x] STEP 03-008: lock-verify-command-conventions

## Goal

Implement lock verify command conventions so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\tools\verify.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\scaffold\verify-commands.test.ts

## Verification

- `bun test test/scaffold/verify-commands.test.ts`
- `bun test`
...
- The targeted test file `test/scaffold/verify-commands.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
