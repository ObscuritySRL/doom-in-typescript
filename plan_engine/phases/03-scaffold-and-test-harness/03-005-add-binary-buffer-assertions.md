# [x] STEP 03-005: add-binary-buffer-assertions

## Goal

Implement add binary buffer assertions so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test_harness\binaryAssert.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test_harness\binary-assert.test.ts

## Verification

- `bun test test/_harness/binary-assert.test.ts`
- `bun test`
...
- The targeted test file `test/_harness/binary-assert.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
