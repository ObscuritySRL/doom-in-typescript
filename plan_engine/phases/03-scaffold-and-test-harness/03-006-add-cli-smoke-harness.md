# [ ] STEP 03-006: add-cli-smoke-harness

## Goal

Implement add cli smoke harness so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test_harness\cli.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\scaffold\cli-smoke.test.ts

## Verification

- `bun test test/scaffold/cli-smoke.test.ts`
- `bun test`
...
- The targeted test file `test/scaffold/cli-smoke.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
