# [ ] STEP 03-002: scaffold-test-layout

## Goal

Implement scaffold test layout so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test_harness\index.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\scaffold\test-layout.test.ts

## Verification

- `bun test test/scaffold/test-layout.test.ts`
- `bun test`
...
- The targeted test file `test/scaffold/test-layout.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
