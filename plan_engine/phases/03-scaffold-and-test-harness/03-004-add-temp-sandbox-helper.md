# [ ] STEP 03-004: add-temp-sandbox-helper

## Goal

Implement add temp sandbox helper so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\test_harness\tempSandbox.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test_harness\temp-sandbox.test.ts

## Verification

- `bun test test/_harness/temp-sandbox.test.ts`
- `bun test`
...
- The targeted test file `test/_harness/temp-sandbox.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
