# [x] STEP 13-011: sprite-projection

## Goal

Implement sprite projection so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\render\spriteProjection.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\render\sprite-projection.test.ts

## Verification

- `bun test test/render/sprite-projection.test.ts`
- `bun test`
...
- The targeted test file `test/render/sprite-projection.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
