# [ ] STEP 13-002: visplane-openings-and-clip-limits

## Goal

Implement visplane openings and clip limits so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\render\renderLimits.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\render\render-limits.test.ts

## Verification

- `bun test test/render/render-limits.test.ts`
- `bun test`
...
- The targeted test file `test/render/render-limits.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
