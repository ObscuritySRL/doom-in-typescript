# [ ] STEP 13-013: masked-midtextures

## Goal

Implement masked midtextures so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\render\maskedTextures.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\render\masked-midtextures.test.ts

## Verification

- `bun test test/render/masked-midtextures.test.ts`
- `bun test`
...
- The targeted test file `test/render/masked-midtextures.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
