# [ ] STEP 13-014: fuzz-and-invisibility

## Goal

Implement fuzz and invisibility so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\render\fuzz.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\render\fuzz.test.ts

## Verification

- `bun test test/render/fuzz.test.ts`
- `bun test`
...
- The targeted test file `test/render/fuzz.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
