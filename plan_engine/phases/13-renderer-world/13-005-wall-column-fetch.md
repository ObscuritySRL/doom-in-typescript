# [ ] STEP 13-005: wall-column-fetch

## Goal

Implement wall column fetch so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\render\wallColumns.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\render\wall-column-fetch.test.ts

## Verification

- `bun test test/render/wall-column-fetch.test.ts`
- `bun test`
...
- The targeted test file `test/render/wall-column-fetch.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
