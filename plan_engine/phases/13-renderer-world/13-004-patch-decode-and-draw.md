# [ ] STEP 13-004: patch-decode-and-draw

## Goal

Implement patch decode and draw so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\render\patchDraw.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\render\patch-draw.test.ts

## Verification

- `bun test test/render/patch-draw.test.ts`
- `bun test`
...
- The targeted test file `test/render/patch-draw.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
