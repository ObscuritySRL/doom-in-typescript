# [ ] STEP 13-008: visplane-build

## Goal

Implement visplane build so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\render\visplanes.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\render\visplane-build.test.ts

## Verification

- `bun test test/render/visplane-build.test.ts`
- `bun test`
...
- The targeted test file `test/render/visplane-build.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
