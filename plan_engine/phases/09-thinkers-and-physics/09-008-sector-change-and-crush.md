# [ ] STEP 09-008: sector-change-and-crush

## Goal

Implement sector change and crush so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\world\sectorChange.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\world\sector-change.test.ts

## Verification

- `bun test test/world/sector-change.test.ts`
- `bun test`
...
- The targeted test file `test/world/sector-change.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
