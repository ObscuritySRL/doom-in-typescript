# [x] STEP 09-005: xy-movement

## Goal

Implement xy movement so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\world\xyMovement.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\world\xy-movement.test.ts

## Verification

- `bun test test/world/xy-movement.test.ts`
- `bun test`
...
- The targeted test file `test/world/xy-movement.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
