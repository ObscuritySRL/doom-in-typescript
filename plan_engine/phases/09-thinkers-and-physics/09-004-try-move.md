# [x] STEP 09-004: try-move

## Goal

Implement try move so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\world\tryMove.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\world\try-move.test.ts

## Verification

- `bun test test/world/try-move.test.ts`
- `bun test`
...
- The targeted test file `test/world/try-move.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
