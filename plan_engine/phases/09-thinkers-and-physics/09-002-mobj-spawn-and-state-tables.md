# [x] STEP 09-002: mobj-spawn-and-state-tables

## Goal

Implement mobj spawn and state tables so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\world\mobj.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\world\mobj-spawn.test.ts

## Verification

- `bun test test/world/mobj-spawn.test.ts`
- `bun test`
...
- The targeted test file `test/world/mobj-spawn.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
