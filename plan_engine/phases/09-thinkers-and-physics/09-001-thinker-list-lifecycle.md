# [ ] STEP 09-001: thinker-list-lifecycle

## Goal

Implement thinker list lifecycle so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\world\thinkers.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\world\thinker-list.test.ts

## Verification

- `bun test test/world/thinker-list.test.ts`
- `bun test`
...
- The targeted test file `test/world/thinker-list.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
