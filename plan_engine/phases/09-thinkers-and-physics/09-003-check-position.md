# [ ] STEP 09-003: check-position

## Goal

Implement check position so it matches the canonical reference behavior and is locked by the listed test coverage.
...
- d:\Projects\bun-win32\doom_codex\src\world\checkPosition.ts

## Test Files

- d:\Projects\bun-win32\doom_codex\test\world\check-position.test.ts

## Verification

- `bun test test/world/check-position.test.ts`
- `bun test`
...
- The targeted test file `test/world/check-position.test.ts` covers the normal path and at least one parity-sensitive edge case, and every listed verification command passes.
