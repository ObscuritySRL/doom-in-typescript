...
import capture from './capture-demo-three-playback-oracle.json';

const STEP_FILE_PATH = 'plan_vanilla_parity/steps/02-030-capture-demo-three-playback-oracle.md';

describe('demo three playback capture', () => {
  test('identity, lane, pending status', () => {
    expect(capture.id).toBe('OR-VP-DEMO3-030');
    expect(capture.stepId).toBe('02-030');
    expect(capture.stepTitle).toBe('Capture Demo Three Playback Oracle');
    expect(capture.lane).toBe('oracle');
    expect(capture.captureStatus).toBe('pending-external-reference-run');
  });

  test('reference targets present at 35 Hz; demo lump is DEMO3', () => {
    ...
    expect(capture.demoLumpName).toBe('DEMO3');
  ...
  test('entries empty while pending; step file pins write lock', async () => {
    ...
    expect(stepFileText).toContain('- test/vanilla_parity/oracles/capture-demo-three-playback-oracle.json');
    expect(stepFileText).toContain('- test/vanilla_parity/oracles/capture-demo-three-playback-oracle.test.ts');
  ...
  test('failure mode: a wrong demo lump name would not match', () => {
    ...
    expect(capture.demoLumpName).not.toBe('DEMO2');
  });
});
