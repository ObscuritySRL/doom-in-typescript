...
const INVENTORY_JSON_PATH = 'plan_vanilla_parity/current-state/inventory-player-modules.json';
const STEP_FILE_PATH = 'plan_vanilla_parity/steps/01-008-inventory-player-modules.md';
const PLAYER_DIRECTORY_PATH = 'src/player/';
const PLAYER_TEST_DIRECTORY_PATH = 'test/player/';
const ALLOWED_GROUP_VALUES: readonly string[] = ['player_attacks', 'player_movement', 'player_pickups', 'player_powerups', 'player_state', 'player_weapons'];
const PLAYER_MODULE_PATHS_SORTED: readonly string[] = [
  'src/player/hitscan.ts',
  'src/player/movement.ts',
  'src/player/pickups.ts',
  'src/player/playerSpawn.ts',
  'src/player/powerups.ts',
  'src/player/projectiles.ts',
  'src/player/weaponStates.ts',
  'src/player/weapons.ts',
];
...
interface InventoryReferenceManifestStatus {
  ...
  readonly vanilla_player_source_files: readonly string[];
  readonly notes: string;
}

interface InventoryPlayerBoundaryStatus {
  readonly player_modules_sorted: readonly string[];
...
interface InventoryDocument {
  ...
  readonly player_boundary_status: InventoryPlayerBoundaryStatus;
...
describe('inventory: player modules', () => {
  ...
  test('inventory declares the canonical id, title, and inventory lane', async () => {
    ...
    expect(inventory.id).toBe('01-008');
    expect(inventory.title).toBe('Inventory Player Modules');
  ...
  test('inventory summary and evidence method identify the audited player surface', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.summary).toContain(PLAYER_DIRECTORY_PATH);
    ...
    expect(inventory.evidence_method).toContain(PLAYER_DIRECTORY_PATH);
  ...
  test('inventory source_directories enumerate src/player with on-disk totals', async () => {
    const inventory = await loadInventoryDocument();
    const observedPaths = inventory.source_directories.map((directoryEntry) => directoryEntry.relative_path);
    expect([...observedPaths]).toEqual([PLAYER_DIRECTORY_PATH]);
  ...
  test('inventory modules are sorted ASCIIbetically and cover the canonical player source paths', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.modules.length).toBe(PLAYER_MODULE_PATHS_SORTED.length);
    const observedPaths = inventory.modules.map((moduleEntry) => moduleEntry.relative_path);
    expectAsciiSorted(observedPaths);
    expect([...observedPaths]).toEqual([...PLAYER_MODULE_PATHS_SORTED]);
  });

  test('inventory module groups partition the player surface into the canonical role buckets', async () => {
    const inventory = await loadInventoryDocument();
    ...
    expect([...grouped.get('player_attacks')!].sort()).toEqual(['src/player/hitscan.ts', 'src/player/projectiles.ts']);
    expect([...grouped.get('player_movement')!].sort()).toEqual(['src/player/movement.ts']);
    expect([...grouped.get('player_pickups')!].sort()).toEqual(['src/player/pickups.ts']);
    expect([...grouped.get('player_powerups')!].sort()).toEqual(['src/player/powerups.ts']);
    expect([...grouped.get('player_state')!].sort()).toEqual(['src/player/playerSpawn.ts']);
    expect([...grouped.get('player_weapons')!].sort()).toEqual(['src/player/weaponStates.ts', 'src/player/weapons.ts']);
  });
  ...
  test('inventory test_coverage matches the committed player test root', async () => {
    const inventory = await loadInventoryDocument();
    const observedRoots = inventory.test_coverage.test_roots.map((testRoot) => testRoot.relative_path);
    expect([...observedRoots]).toEqual([PLAYER_TEST_DIRECTORY_PATH]);
    ...
    expect(inventory.test_coverage.notes).toContain('test/player/');
  });
  ...
  test('inventory reference_manifest_status matches local vanilla limit and source catalog manifests without embedding WAD bytes', async () => {
    ...
    expectAsciiSorted(status.vanilla_player_source_files);
    const catalogSourcePaths = sourceCatalog.entries.map((entry: { source: string }) => entry.source);
    for (const vanillaSourceFile of status.vanilla_player_source_files) {
  ...
  test('inventory player_boundary_status enumerates every player module and preserves source-only metadata status', async () => {
    const inventory = await loadInventoryDocument();
    const status = inventory.player_boundary_status;
    expectAsciiSorted(status.player_modules_sorted);
    expect([...status.player_modules_sorted]).toEqual([...PLAYER_MODULE_PATHS_SORTED]);
    expect(status.runtime_module_count).toBe(PLAYER_MODULE_PATHS_SORTED.length);
    expect(status.source_only_metadata).toBe(true);
    expect(status.notes).toContain('src/player/');
  });

  test('inventory implications reference player record reuse, attack callback wiring, DEH defaults, and downstream player gates', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.implications.length).toBeGreaterThan(0);
    const concatenatedImplications = inventory.implications.join('\n');
    expect(concatenatedImplications).toContain(PLAYER_DIRECTORY_PATH);
    expect(concatenatedImplications).toContain('Player');
    expect(concatenatedImplications).toContain('callback');
    expect(concatenatedImplications).toContain('DEH_');
    expect(concatenatedImplications).toContain('07-');
  });
  ...
  test('step file under plan_vanilla_parity/steps pins the same write lock paths as this test enforces', async () => {
    ...
    expect(stepText).toContain('test/vanilla_parity/current-state/inventory-player-modules.test.ts');
    expect(stepText).toContain('\n## lane\n\ninventory\n');
  });

  test('failure mode: a tampered inventory that marks a player attack module as state plumbing diverges from the captured snapshot', async () => {
    const inventory = await loadInventoryDocument();
    const originalEntry = inventory.modules.find((moduleEntry) => moduleEntry.relative_path === 'src/player/hitscan.ts')!;
  ...
  test('failure mode: a test coverage snapshot that drops the player root diverges from the captured test surface', async () => {
    const inventory = await loadInventoryDocument();
    const tamperedCoverage: InventoryTestCoverage = {
      ...inventory.test_coverage,
      test_roots: inventory.test_coverage.test_roots.filter((testRoot) => testRoot.relative_path !== PLAYER_TEST_DIRECTORY_PATH),
    };
    expect(tamperedCoverage.test_roots.length).not.toBe(inventory.test_coverage.test_roots.length);
    expect(tamperedCoverage.test_roots.map((testRoot) => testRoot.relative_path)).not.toContain(PLAYER_TEST_DIRECTORY_PATH);
  });
});
