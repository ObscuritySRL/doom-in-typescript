...
import demoLumpSummary from '../../../reference/manifests/demo-lump-summary.json';

const INVENTORY_JSON_PATH = 'plan_vanilla_parity/current-state/inventory-demo-and-replay-modules.json';
const STEP_FILE_PATH = 'plan_vanilla_parity/steps/01-014-inventory-demo-and-replay-modules.md';
const DEMO_DIRECTORY_PATH = 'src/demo/';
const DEMO_TEST_DIRECTORY_PATH = 'test/demo/';
const ALLOWED_GROUP_VALUES: readonly string[] = ['demo_lump', 'demo_parse', 'demo_playback', 'demo_record'];
const DEMO_MODULE_PATHS_SORTED: readonly string[] = ['src/demo/demoFile.ts', 'src/demo/demoParse.ts', 'src/demo/demoPlayback.ts', 'src/demo/demoRecord.ts'];
...
interface InventoryTestCoverage {
  ...
  readonly cross_lane_test_files: readonly string[];
  readonly notes: string;
}

interface InventoryReferenceManifestStatus {
  readonly vanilla_limit_summary_relative_path: string;
  readonly maxdemosize_limit: number;
  readonly maxplayers_limit: number;
  readonly ticrate_constant: number;
  readonly demo_version_19_constant: number;
  readonly demo_long_tics_version_constant: number;
  readonly demo_end_marker_constant: number;
  readonly demo_lump_summary_relative_path: string;
  readonly demo_lump_count: number;
  readonly demo_lump_names_sorted: readonly string[];
  readonly source_catalog_relative_path: string;
  readonly vanilla_demo_source_files: readonly string[];
  readonly doomwiki_demo_doc_id: string;
  readonly notes: string;
}

interface InventoryDemoAndReplayBoundaryStatus {
  readonly demo_modules_sorted: readonly string[];
  readonly runtime_module_count: number;
  readonly parser_module_count: number;
  readonly playback_module_count: number;
  readonly record_module_count: number;
...
interface InventoryDocument {
  ...
  readonly demo_and_replay_boundary_status: InventoryDemoAndReplayBoundaryStatus;
...
describe('inventory: demo and replay modules', () => {
  ...
  test('inventory declares the canonical id, title, and inventory lane', async () => {
    ...
    expect(inventory.id).toBe('01-014');
    expect(inventory.title).toBe('Inventory Demo And Replay Modules');
  ...
  test('inventory summary and evidence method identify the audited demo and replay surface', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.summary).toContain(DEMO_DIRECTORY_PATH);
    ...
    expect(inventory.evidence_method).toContain(DEMO_DIRECTORY_PATH);
    ...
    expect(inventory.evidence_method).toContain('reference/manifests/demo-lump-summary.json');
  });
  ...
  test('inventory source_directories enumerate src/demo with on-disk totals', async () => {
    const inventory = await loadInventoryDocument();
    const observedPaths = inventory.source_directories.map((directoryEntry) => directoryEntry.relative_path);
    expect([...observedPaths]).toEqual([DEMO_DIRECTORY_PATH]);
  ...
  test('inventory modules are sorted ASCIIbetically and cover the canonical demo and replay source paths', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.modules.length).toBe(DEMO_MODULE_PATHS_SORTED.length);
    const observedPaths = inventory.modules.map((moduleEntry) => moduleEntry.relative_path);
    expectAsciiSorted(observedPaths);
    expect([...observedPaths]).toEqual([...DEMO_MODULE_PATHS_SORTED]);
  });

  test('inventory module groups partition the demo and replay surface into the canonical role buckets', async () => {
    const inventory = await loadInventoryDocument();
    ...
    expect([...grouped.get('demo_lump')!].sort()).toEqual(['src/demo/demoFile.ts']);
    expect([...grouped.get('demo_parse')!].sort()).toEqual(['src/demo/demoParse.ts']);
    expect([...grouped.get('demo_playback')!].sort()).toEqual(['src/demo/demoPlayback.ts']);
    expect([...grouped.get('demo_record')!].sort()).toEqual(['src/demo/demoRecord.ts']);
  });
  ...
  test('inventory test_coverage matches the committed demo test root and references cross-lane parity and replay tests', async () => {
    const inventory = await loadInventoryDocument();
    const observedRoots = inventory.test_coverage.test_roots.map((testRoot) => testRoot.relative_path);
    expect([...observedRoots]).toEqual([DEMO_TEST_DIRECTORY_PATH]);
    ...
    expectAsciiSorted(inventory.test_coverage.cross_lane_test_files);
    for (const crossLaneTestFile of inventory.test_coverage.cross_lane_test_files) {
      expect(existsSync(crossLaneTestFile)).toBe(true);
      expect(statSync(crossLaneTestFile).isFile()).toBe(true);
    }
    expect(inventory.test_coverage.notes).toContain('test/demo/');
    expect(inventory.test_coverage.notes).toContain('test/parity/bundled-demo-sync.test.ts');
    expect(inventory.test_coverage.notes).toContain('test/playable/demo-replay/');
  });

  test('inventory reference_manifest_status matches local vanilla limit, demo lump, and source catalog manifests without embedding WAD bytes', async () => {
    const inventory = await loadInventoryDocument();
    ...
    expect(status.maxdemosize_limit).toBe(findLimitValue('MAXDEMOSIZE'));
    expect(status.maxplayers_limit).toBe(findLimitValue('MAXPLAYERS'));
    expect(status.ticrate_constant).toBe(35);
    expect(status.demo_version_19_constant).toBe(109);
    expect(status.demo_long_tics_version_constant).toBe(111);
    expect(status.demo_end_marker_constant).toBe(128);
    expect(status.demo_lump_summary_relative_path).toBe('reference/manifests/demo-lump-summary.json');
    expect(status.demo_lump_count).toBe(demoLumpSummary.demos.length);
    expectAsciiSorted(status.demo_lump_names_sorted);
    const expectedDemoLumpNames = demoLumpSummary.demos.map((demoEntry) => demoEntry.name).sort();
    expect([...status.demo_lump_names_sorted]).toEqual([...expectedDemoLumpNames]);
    expect(status.source_catalog_relative_path).toBe('reference/manifests/source-catalog.json');
    expectAsciiSorted(status.vanilla_demo_source_files);
    const catalogSourcePaths = sourceCatalog.entries.map((entry: { source: string }) => entry.source);
    for (const vanillaSourceFile of status.vanilla_demo_source_files) {
      const matchingEntry = catalogSourcePaths.find((sourcePath: string) => sourcePath.endsWith(`/${vanillaSourceFile}`));
      expect(matchingEntry).toBeDefined();
    }
    const doomwikiEntry = sourceCatalog.entries.find((entry: { id: string }) => entry.id === status.doomwiki_demo_doc_id);
    expect(doomwikiEntry).toBeDefined();
    expect(status.notes).toContain('does not embed raw');
  });

  test('inventory demo_and_replay_boundary_status enumerates every demo module and preserves source-only metadata status', async () => {
    const inventory = await loadInventoryDocument();
    const status = inventory.demo_and_replay_boundary_status;
    expectAsciiSorted(status.demo_modules_sorted);
    expect([...status.demo_modules_sorted]).toEqual([...DEMO_MODULE_PATHS_SORTED]);
    expect(status.runtime_module_count).toBe(DEMO_MODULE_PATHS_SORTED.length);
    expect(status.parser_module_count).toBe(DEMO_MODULE_PATHS_SORTED.filter((modulePath) => modulePath === 'src/demo/demoFile.ts' || modulePath === 'src/demo/demoParse.ts').length);
    expect(status.playback_module_count).toBe(DEMO_MODULE_PATHS_SORTED.filter((modulePath) => modulePath === 'src/demo/demoPlayback.ts').length);
    expect(status.record_module_count).toBe(DEMO_MODULE_PATHS_SORTED.filter((modulePath) => modulePath === 'src/demo/demoRecord.ts').length);
    expect(status.source_only_metadata).toBe(true);
    expect(status.notes).toContain('src/demo/');
  });

  test('inventory implications reference cross-lane reuse, replay surface, vanilla constants, and downstream demo gates', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.implications.length).toBeGreaterThan(0);
    const concatenatedImplications = inventory.implications.join('\n');
    expect(concatenatedImplications).toContain(DEMO_DIRECTORY_PATH);
    expect(concatenatedImplications).toContain('DemoPlayback');
    expect(concatenatedImplications).toContain('DemoRecorder');
    expect(concatenatedImplications).toContain('DEMO_VANILLA_VERSION_19');
    expect(concatenatedImplications).toContain('MAXDEMOSIZE');
    expect(concatenatedImplications).toContain('04-');
  });
  ...
  test('step file under plan_vanilla_parity/steps pins the same write lock paths as this test enforces', async () => {
    ...
    expect(stepText).toContain('test/vanilla_parity/current-state/inventory-demo-and-replay-modules.test.ts');
    expect(stepText).toContain('\n## lane\n\ninventory\n');
  });

  test('failure mode: a tampered inventory that marks the demo recorder as a demo playback module diverges from the captured snapshot', async () => {
    const inventory = await loadInventoryDocument();
    const originalEntry = inventory.modules.find((moduleEntry) => moduleEntry.relative_path === 'src/demo/demoRecord.ts')!;
  ...
  test('failure mode: a tampered reference manifest status with a wrong MAXDEMOSIZE limit diverges from the manifest', async () => {
    const inventory = await loadInventoryDocument();
    const tamperedStatus: InventoryReferenceManifestStatus = {
      ...inventory.reference_manifest_status,
      maxdemosize_limit: inventory.reference_manifest_status.maxdemosize_limit + 1,
    };
    expect(tamperedStatus.maxdemosize_limit).not.toBe(findLimitValue('MAXDEMOSIZE'));
  });

  test('failure mode: a test coverage snapshot that drops the demo test root diverges from the captured test surface', async () => {
    const inventory = await loadInventoryDocument();
    const tamperedCoverage: InventoryTestCoverage = {
      ...inventory.test_coverage,
      test_roots: inventory.test_coverage.test_roots.filter((testRoot) => testRoot.relative_path !== DEMO_TEST_DIRECTORY_PATH),
    };
    expect(tamperedCoverage.test_roots.length).not.toBe(inventory.test_coverage.test_roots.length);
    expect(tamperedCoverage.test_roots.map((testRoot) => testRoot.relative_path)).not.toContain(DEMO_TEST_DIRECTORY_PATH);
  });
});
