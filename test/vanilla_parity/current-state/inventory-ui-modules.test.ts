...
const INVENTORY_JSON_PATH = 'plan_vanilla_parity/current-state/inventory-ui-modules.json';
const STEP_FILE_PATH = 'plan_vanilla_parity/steps/01-011-inventory-ui-modules.md';
const UI_DIRECTORY_PATH = 'src/ui/';
const UI_TEST_DIRECTORY_PATH = 'test/ui/';
const ALLOWED_GROUP_VALUES: readonly string[] = ['ui_assets', 'ui_front_end', 'ui_in_game', 'ui_session'];
const UI_MODULE_PATHS_SORTED: readonly string[] = [
  'src/ui/assets.ts',
  'src/ui/automap.ts',
  'src/ui/endoom.ts',
  'src/ui/finale.ts',
  'src/ui/frontEndSequence.ts',
  'src/ui/hudMessages.ts',
...
interface InventoryReferenceManifestStatus {
  ...
  readonly vanilla_ui_source_files: readonly string[];
  readonly notes: string;
}

interface InventoryUiBoundaryStatus {
  readonly ui_modules_sorted: readonly string[];
...
interface InventoryDocument {
  ...
  readonly ui_boundary_status: InventoryUiBoundaryStatus;
...
describe('inventory: ui modules', () => {
  ...
  test('inventory declares the canonical id, title, and inventory lane', async () => {
    ...
    expect(inventory.id).toBe('01-011');
    expect(inventory.title).toBe('Inventory UI Modules');
  ...
  test('inventory summary and evidence method identify the audited ui surface', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.summary).toContain(UI_DIRECTORY_PATH);
    ...
    expect(inventory.evidence_method).toContain(UI_DIRECTORY_PATH);
  ...
  test('inventory source_directories enumerate src/ui with on-disk totals', async () => {
    const inventory = await loadInventoryDocument();
    const observedPaths = inventory.source_directories.map((directoryEntry) => directoryEntry.relative_path);
    expect([...observedPaths]).toEqual([UI_DIRECTORY_PATH]);
  ...
  test('inventory modules are sorted ASCIIbetically and cover the canonical ui source paths', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.modules.length).toBe(UI_MODULE_PATHS_SORTED.length);
    const observedPaths = inventory.modules.map((moduleEntry) => moduleEntry.relative_path);
    expectAsciiSorted(observedPaths);
    expect([...observedPaths]).toEqual([...UI_MODULE_PATHS_SORTED]);
  });

  test('inventory module groups partition the ui surface into the canonical role buckets', async () => {
    const inventory = await loadInventoryDocument();
    ...
    expect([...grouped.get('ui_assets')!].sort()).toEqual(['src/ui/assets.ts']);
    expect([...grouped.get('ui_front_end')!].sort()).toEqual(['src/ui/endoom.ts', 'src/ui/frontEndSequence.ts', 'src/ui/menus.ts']);
    expect([...grouped.get('ui_in_game')!].sort()).toEqual(['src/ui/automap.ts', 'src/ui/hudMessages.ts', 'src/ui/statusBar.ts']);
    expect([...grouped.get('ui_session')!].sort()).toEqual(['src/ui/finale.ts', 'src/ui/intermission.ts']);
  });
  ...
  test('inventory test_coverage matches the committed ui test root', async () => {
    const inventory = await loadInventoryDocument();
    const observedRoots = inventory.test_coverage.test_roots.map((testRoot) => testRoot.relative_path);
    expect([...observedRoots]).toEqual([UI_TEST_DIRECTORY_PATH]);
    ...
    expect(inventory.test_coverage.notes).toContain('test/ui/');
  });
  ...
  test('inventory reference_manifest_status matches local vanilla limit and source catalog manifests without embedding WAD bytes', async () => {
    ...
    expectAsciiSorted(status.vanilla_ui_source_files);
    const catalogSourcePaths = sourceCatalog.entries.map((entry: { source: string }) => entry.source);
    for (const vanillaSourceFile of status.vanilla_ui_source_files) {
  ...
  test('inventory ui_boundary_status enumerates every ui module and preserves source-only metadata status', async () => {
    const inventory = await loadInventoryDocument();
    const status = inventory.ui_boundary_status;
    expectAsciiSorted(status.ui_modules_sorted);
    expect([...status.ui_modules_sorted]).toEqual([...UI_MODULE_PATHS_SORTED]);
    expect(status.runtime_module_count).toBe(UI_MODULE_PATHS_SORTED.length);
    expect(status.source_only_metadata).toBe(true);
    expect(status.notes).toContain('src/ui/');
  });

  test('inventory implications reference cross-lane reuse, asset catalog wiring, vanilla constants, and downstream ui gates', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.implications.length).toBeGreaterThan(0);
    const concatenatedImplications = inventory.implications.join('\n');
    expect(concatenatedImplications).toContain(UI_DIRECTORY_PATH);
    expect(concatenatedImplications).toContain('MenuState');
    expect(concatenatedImplications).toContain('FrontEndSequenceState');
    expect(concatenatedImplications).toContain('SAVESTRINGSIZE');
    expect(concatenatedImplications).toContain('10-');
  });
  ...
  test('step file under plan_vanilla_parity/steps pins the same write lock paths as this test enforces', async () => {
    ...
    expect(stepText).toContain('test/vanilla_parity/current-state/inventory-ui-modules.test.ts');
    expect(stepText).toContain('\n## lane\n\ninventory\n');
  });

  test('failure mode: a tampered inventory that marks a front-end module as a session module diverges from the captured snapshot', async () => {
    const inventory = await loadInventoryDocument();
    const originalEntry = inventory.modules.find((moduleEntry) => moduleEntry.relative_path === 'src/ui/menus.ts')!;
  ...
  test('failure mode: a tampered reference manifest status with a wrong SAVESTRINGSIZE limit diverges from the manifest', async () => {
    const inventory = await loadInventoryDocument();
    const tamperedStatus: InventoryReferenceManifestStatus = {
      ...inventory.reference_manifest_status,
      savestringsize_limit: inventory.reference_manifest_status.savestringsize_limit + 1,
    };
    expect(tamperedStatus.savestringsize_limit).not.toBe(findLimitValue('SAVESTRINGSIZE'));
  });

  test('failure mode: a test coverage snapshot that drops the ui test root diverges from the captured test surface', async () => {
    const inventory = await loadInventoryDocument();
    const tamperedCoverage: InventoryTestCoverage = {
      ...inventory.test_coverage,
      test_roots: inventory.test_coverage.test_roots.filter((testRoot) => testRoot.relative_path !== UI_TEST_DIRECTORY_PATH),
    };
    expect(tamperedCoverage.test_roots.length).not.toBe(inventory.test_coverage.test_roots.length);
    expect(tamperedCoverage.test_roots.map((testRoot) => testRoot.relative_path)).not.toContain(UI_TEST_DIRECTORY_PATH);
  });
});
