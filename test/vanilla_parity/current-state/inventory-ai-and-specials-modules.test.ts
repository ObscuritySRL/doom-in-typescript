...
const INVENTORY_JSON_PATH = 'plan_vanilla_parity/current-state/inventory-ai-and-specials-modules.json';
const STEP_FILE_PATH = 'plan_vanilla_parity/steps/01-009-inventory-ai-and-specials-modules.md';
const AI_DIRECTORY_PATH = 'src/ai/';
const SPECIALS_DIRECTORY_PATH = 'src/specials/';
const AI_TEST_DIRECTORY_PATH = 'test/ai/';
const SPECIALS_TEST_DIRECTORY_PATH = 'test/specials/';
const ALLOWED_GROUP_VALUES: readonly string[] = ['ai_attacks', 'ai_navigation', 'ai_state', 'ai_targeting', 'specials_environment', 'specials_lines', 'specials_persistence', 'specials_planes'];
const AI_MODULE_PATHS_SORTED: readonly string[] = [
  'src/ai/attacks.ts',
...
const SPECIALS_MODULE_PATHS_SORTED: readonly string[] = [
  'src/specials/activeSpecials.ts',
  'src/specials/animations.ts',
  'src/specials/ceilings.ts',
  'src/specials/doors.ts',
  'src/specials/floors.ts',
  'src/specials/lineTriggers.ts',
  'src/specials/platforms.ts',
  'src/specials/sectorSpecials.ts',
  'src/specials/stairsDonut.ts',
  'src/specials/switches.ts',
];
const ALL_MODULE_PATHS_SORTED: readonly string[] = [...AI_MODULE_PATHS_SORTED, ...SPECIALS_MODULE_PATHS_SORTED];
...
interface InventoryReferenceManifestStatus {
  ...
  readonly maxswitches_limit: number;
  readonly maxanims_limit: number;
  readonly buttontime_tics: number;
  readonly ticrate_constant: number;
  readonly source_catalog_relative_path: string;
  readonly vanilla_ai_and_specials_source_files: readonly string[];
  readonly notes: string;
}

interface InventoryAiAndSpecialsBoundaryStatus {
  readonly ai_modules_sorted: readonly string[];
  readonly specials_modules_sorted: readonly string[];
  readonly ai_module_count: number;
  readonly specials_module_count: number;
...
interface InventoryDocument {
  ...
  readonly ai_and_specials_boundary_status: InventoryAiAndSpecialsBoundaryStatus;
...
describe('inventory: ai and specials modules', () => {
  ...
  test('inventory declares the canonical id, title, and inventory lane', async () => {
    ...
    expect(inventory.id).toBe('01-009');
    expect(inventory.title).toBe('Inventory AI And Specials Modules');
  ...
  test('inventory summary and evidence method identify the audited ai and specials surface', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.summary).toContain(AI_DIRECTORY_PATH);
    expect(inventory.summary).toContain(SPECIALS_DIRECTORY_PATH);
    ...
    expect(inventory.evidence_method).toContain(AI_DIRECTORY_PATH);
    expect(inventory.evidence_method).toContain(SPECIALS_DIRECTORY_PATH);
  ...
  test('inventory source_directories enumerate src/ai and src/specials with on-disk totals', async () => {
    const inventory = await loadInventoryDocument();
    const observedPaths = inventory.source_directories.map((directoryEntry) => directoryEntry.relative_path);
    expect([...observedPaths]).toEqual([AI_DIRECTORY_PATH, SPECIALS_DIRECTORY_PATH]);
  ...
  test('inventory modules are sorted ASCIIbetically and cover the canonical ai and specials source paths', async () => {
    const inventory = await loadInventoryDocument();
  ...
  test('inventory module groups partition the ai and specials surface into the canonical role buckets', async () => {
    const inventory = await loadInventoryDocument();
    ...
    expect([...grouped.get('ai_attacks')!].sort()).toEqual(['src/ai/attacks.ts', 'src/ai/bossSpecials.ts']);
    expect([...grouped.get('ai_navigation')!].sort()).toEqual(['src/ai/chase.ts', 'src/ai/soundPropagation.ts']);
    expect([...grouped.get('ai_state')!].sort()).toEqual(['src/ai/stateTransitions.ts']);
    expect([...grouped.get('ai_targeting')!].sort()).toEqual(['src/ai/meleeRange.ts', 'src/ai/missileRange.ts', 'src/ai/targeting.ts']);
    expect([...grouped.get('specials_environment')!].sort()).toEqual(['src/specials/animations.ts', 'src/specials/sectorSpecials.ts']);
    expect([...grouped.get('specials_lines')!].sort()).toEqual(['src/specials/lineTriggers.ts', 'src/specials/switches.ts']);
    expect([...grouped.get('specials_persistence')!].sort()).toEqual(['src/specials/activeSpecials.ts']);
    expect([...grouped.get('specials_planes')!].sort()).toEqual(['src/specials/ceilings.ts', 'src/specials/doors.ts', 'src/specials/floors.ts', 'src/specials/platforms.ts', 'src/specials/stairsDonut.ts']);
  });
  ...
  test('inventory test_coverage matches the committed ai and specials test roots', async () => {
    const inventory = await loadInventoryDocument();
    const observedRoots = inventory.test_coverage.test_roots.map((testRoot) => testRoot.relative_path);
    expect([...observedRoots]).toEqual([AI_TEST_DIRECTORY_PATH, SPECIALS_TEST_DIRECTORY_PATH]);
    ...
    expect(inventory.test_coverage.notes).toContain('test/ai/');
    expect(inventory.test_coverage.notes).toContain('test/specials/');
  });
  ...
  test('inventory reference_manifest_status matches local vanilla limit and source catalog manifests without embedding WAD bytes', async () => {
    ...
    expect(status.maxanims_limit).toBe(findLimitValue('MAXANIMS'));
    expect(status.maxswitches_limit).toBe(50);
    expect(status.buttontime_tics).toBe(35);
    ...
    expectAsciiSorted(status.vanilla_ai_and_specials_source_files);
    const catalogSourcePaths = sourceCatalog.entries.map((entry: { source: string }) => entry.source);
    for (const vanillaSourceFile of status.vanilla_ai_and_specials_source_files) {
  ...
  test('inventory ai_and_specials_boundary_status enumerates every ai and specials module and preserves source-only metadata status', async () => {
    const inventory = await loadInventoryDocument();
    const status = inventory.ai_and_specials_boundary_status;
    expectAsciiSorted(status.ai_modules_sorted);
    expectAsciiSorted(status.specials_modules_sorted);
    expect([...status.ai_modules_sorted]).toEqual([...AI_MODULE_PATHS_SORTED]);
    expect([...status.specials_modules_sorted]).toEqual([...SPECIALS_MODULE_PATHS_SORTED]);
    expect(status.ai_module_count).toBe(AI_MODULE_PATHS_SORTED.length);
    expect(status.specials_module_count).toBe(SPECIALS_MODULE_PATHS_SORTED.length);
    expect(status.source_only_metadata).toBe(true);
    expect(status.notes).toContain('src/ai/');
    expect(status.notes).toContain('src/specials/');
  });

  test('inventory implications reference cross-lane reuse, callback wiring, vanilla constant tables, and downstream gates', async () => {
    const inventory = await loadInventoryDocument();
    expect(inventory.implications.length).toBeGreaterThan(0);
    const concatenatedImplications = inventory.implications.join('\n');
    expect(concatenatedImplications).toContain(AI_DIRECTORY_PATH);
    expect(concatenatedImplications).toContain(SPECIALS_DIRECTORY_PATH);
    expect(concatenatedImplications).toContain('callback');
    expect(concatenatedImplications).toContain('PlaneMoveResult');
    expect(concatenatedImplications).toContain('MELEERANGE');
    expect(concatenatedImplications).toContain('08-');
  });
  ...
  test('step file under plan_vanilla_parity/steps pins the same write lock paths as this test enforces', async () => {
    ...
    expect(stepText).toContain('test/vanilla_parity/current-state/inventory-ai-and-specials-modules.test.ts');
    expect(stepText).toContain('\n## lane\n\ninventory\n');
  });

  test('failure mode: a tampered inventory that marks a specials plane thinker as a sound state module diverges from the captured snapshot', async () => {
    const inventory = await loadInventoryDocument();
    const originalEntry = inventory.modules.find((moduleEntry) => moduleEntry.relative_path === 'src/specials/doors.ts')!;
  ...
  test('failure mode: a tampered reference manifest status with a wrong MAXCEILINGS limit diverges from the manifest', async () => {
    const inventory = await loadInventoryDocument();
    const tamperedStatus: InventoryReferenceManifestStatus = {
      ...inventory.reference_manifest_status,
      maxceilings_limit: inventory.reference_manifest_status.maxceilings_limit + 1,
    };
    expect(tamperedStatus.maxceilings_limit).not.toBe(findLimitValue('MAXCEILINGS'));
  });

  test('failure mode: a test coverage snapshot that drops the specials root diverges from the captured test surface', async () => {
    const inventory = await loadInventoryDocument();
    const tamperedCoverage: InventoryTestCoverage = {
      ...inventory.test_coverage,
      test_roots: inventory.test_coverage.test_roots.filter((testRoot) => testRoot.relative_path !== SPECIALS_TEST_DIRECTORY_PATH),
    };
    expect(tamperedCoverage.test_roots.length).not.toBe(inventory.test_coverage.test_roots.length);
    expect(tamperedCoverage.test_roots.map((testRoot) => testRoot.relative_path)).not.toContain(SPECIALS_TEST_DIRECTORY_PATH);
  });
});
